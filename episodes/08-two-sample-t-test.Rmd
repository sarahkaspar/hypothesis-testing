...
mice %>% 
  ggplot(aes(Diet,Bodyweight))+
  geom_jitter(width=0.1)+
  theme_bw()+
  labs(title="Scenario for a two-sample t-test")
```
...
library(reshape2)
library(patchwork)
before <- rnorm(n=15, mean=25, sd=5)
after <- before + rnorm(n=15, mean=2, sd=1)

paired_mice <- data.frame(ID=1:15,before, after)
paired_mice <- melt(paired_mice, value.name="Bodyweight", variable.name = "Time",id.vars = "ID")

ggplot(paired_mice, aes(x=Time, y=Bodyweight)) +
  geom_jitter(width=0.05)+
  scale_y_continuous(limits=c(0,max(paired_mice$Bodyweight)))+
  theme_classic()
```
...
unpaired_plot <- ggplot(paired_mice, aes(x=Time, y=Bodyweight)) +
  geom_jitter(width=0.05)+
  scale_y_continuous(limits=c(0,max(paired_mice$Bodyweight)))+
  theme_classic()+
  labs(title="unpaired measurements")
#ggsave("unpaired_plot.pdf",width=3,height=3)

paired_plot <- ggplot(paired_mice, aes(x=Time, y=Bodyweight)) +
  geom_point()+
  scale_y_continuous(limits=c(0,max(paired_mice$Bodyweight)))+
  theme_classic()+
  geom_line(aes(group=ID),colour="gray")+
  labs(title="paired measurements")
#ggsave("paired_plot.pdf",width=3,height=3)

diff = after - before
diff_plot <- ggplot(data.frame(ID=1:15, weight_difference = diff), aes(x=ID, y=weight_difference)) +
  geom_point()+
  geom_hline(yintercept=0)+
  scale_y_continuous(limits=c(-max(diff), max(diff)))+
  theme_classic()+
  labs(title="individual weight gain")

unpaired_plot + paired_plot + diff_plot
```
...
```{r setup-paired-t-test, echo=TRUE, eval=FALSE}
...
1. plotting the data
```{r oxygen-plot-solution, eval=FALSE}
oxygen_data %>% 
...
```{r oxygen-test-solution, eval=FALSE}
